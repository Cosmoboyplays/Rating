...
@app.route('/all_reviews/<string:id_area>/')
@login_required
def all_reviews(id_area):
    with session_maker() as db_session:
        result = db_session.query(Reviews).filter_by(id_area=id_area).order_by(Reviews.date.desc()).all()
        reviews = []
...
@app.route('/last_review/<string:id_area>/')
@login_required
def last_review(id_area):
    with session_maker() as db_session:
        last_review = db_session.query(Reviews).filter_by(id_area=id_area).order_by(Reviews.date.desc()).first()
